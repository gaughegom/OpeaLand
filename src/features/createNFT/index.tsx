...
export default function CreateNFT() {
  const defaultCollection = {
    token: contractAddresses.erc721,
    name: "Default",
    symbol: "",
    amount: 0,
    creator: "",
  ...
  const handleSaveClick = async () => {
    ...
    if (collectionInput === contractAddresses.erc721) {
      var defaultContract = new ethers.Contract(
        contractAddresses.erc721Default,
        ERC721Default.abi,
        currentSigner
      );

      const txMint = await defaultContract.mint("ipfs");
      const txReceipt = await txMint.wait();
      tokenId = txReceipt.events[2].args[1].toString();
    } else {
      ...
      const txMint = await erc721Contract.mint(currentAddress, "ipfs");
      const txReceipt = await txMint.wait();
      tokenId = txReceipt.events[1].args[1].toString();
    }
    ...
    if (statusInput === 1) {
      ...
      const txList = await exchangeSellContract.list(
        collectionInput,
        tokenId,
